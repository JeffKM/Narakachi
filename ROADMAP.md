...
- [ ] **T23** 버그픽스 + 밸런스 + **테스트 재설계** — ① **데모 시드 내러티브 결합 제거**: `DEMO_SEED_AFFINITY`/`_apply_demo_seed` 들어내고 `SaveManager.build_state({...})` 파라미터 상태 빌더 신설 → `test_phase2`/`test_cutin`를 빌더 위로 재작성(매직넘버 단언 제거), 디버그 키 2 = `dev_preset_*` 프리셋(이름에서 "demo" 제거), 게임 로직(`relationship_stage`/컷인)은 보존. ② **실기기 패스**(T24 배포 뒤): 모바일 웹 한글 IME·세이브 영속(IndexedDB/사파리)·공유(Web Share/`JavaScriptBridge`)·터치 히트 영역. ③ 밸런스: 세션 1~3분 확인. ※ 게이지 시드로 라이브 체키 연출하는 데모 연출은 **일단 보류**.
