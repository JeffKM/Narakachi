...
- **옥자 표정/포즈 전신 스탠딩 5~6장**(기본/웃음/부끄/시무룩/제조중/말하기) — 기본 의상 = 마녀룩, 전신 `~128×288`, 같은 프레이밍
...
- [ ] **T06** 메인 화면 레이아웃 + **게임기 셸**(다마고치형 달걀 바디 + 270×480 화면 + 버튼 덱) + **3버튼 입력 매핑**(SELECT 이동·OK 확인·CANCEL 취소) + 화면 터치 하이브리드 + **한 줄 티커**
...
- 🎨 **A1** 옥자 표정 전신 스탠딩 5~6장(기본 의상) + 나라카 지옥 배경
