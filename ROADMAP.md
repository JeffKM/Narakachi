...
- [ ] **T23** 버그픽스 + 밸런스 + **테스트 재설계** — 합의 2026-06-07(grill-me). **헤드리스 범위 완료**(165통과/0실패), 남은 건 실기기 디바이스 세션.
  ...
  - ✅ ② **실기기 패스 — 부분 방어선(헤드리스)**: ✅ 웹 한글 렌더(→T24) / ✅ **영속 직렬화 정합**(D 라운드트립+손상폴백 = IndexedDB/사파리 세이브 손실 1차 방어) / ✅ **공유 합성 스모크**(`ShareCard` 합성→네이티브 PNG 저장, 무크래시·헤드리스 무렌더 시 명시 LOG). **실기기 실측 남음**(디바이스 세션): 모바일 한글 IME·IndexedDB/사파리 실영속·Web Share/`JavaScriptBridge` 다운로드·터치 히트 영역.
