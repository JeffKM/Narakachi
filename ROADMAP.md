...
**📍 추적 이슈 — 멜 슬라이스 (최후 메인, 펫 2마리 선아·수아)**

> 토대(레지스트리·`Meters` 제네릭·로스터·컬렉션 섹션·제네릭 펫 경로)는 미호/바나에서 확립됨 → 멜은 **데이터 1항 + 에셋 + 대사** 위주(미호 트레이서·로스터 불필요). **펫이 2마리(선아·수아)인 첫 슬라이스** — 제네릭 펫 경로(시온이/규종이/코코)에 데이터 2항만 추가(코드 무수정 자동 확장 기대). 멜=강시 메이드(블루/틸 강시 관복 그대로·시그니처 청운 에이드·accent 청록/틸·인트로 지뢰계).

부모 추적: **[#12](https://github.com/JeffKM/Narakuchi/issues/12)** (`gh issue view 12`)

| 이슈 | 슬라이스 | 타입 | 차단 | 상태 |
|---|---|---|---|---|
| [#13](https://github.com/JeffKM/Narakuchi/issues/13) | 🎨 멜·선아·수아 아트 에셋 생산 (멜 표정 6 + 펫 반응 4×2 + 초상 3 + 인트로 체키 3) | HITL | — | ✅ 완료 — 멜·선아·수아 에셋 전량 추가(매니페스트 A13/A14/A15)·헤드리스 로드 검증 |
| [#14](https://github.com/JeffKM/Narakuchi/issues/14) | 🧧 멜 메인 배선 (라이브·교감 파리티·반말 컷인·체키·탭) | AFK | #13 | ✅ 완료 (→ T38) — `buttons.mel` emotion·멜 전용 ticker/talk/gifts/`mel_cutin`(강시 톤)·`buttons:"mel"`·컬렉션 탭 메인 섹션 해제·`docs/script-mel.md`. 헤드리스 392통과 |
| [#15](https://github.com/JeffKM/Narakuchi/issues/15) | 🐩 펫 선아 슬라이스 (제네릭 펫 미러) | AFK | #13, #14 | ✅ 완료 — 레지스트리·`GAUGE_SUNA`·버튼·티커·컬렉션 탭·지뢰계 베이크컷 |
| [#16](https://github.com/JeffKM/Narakuchi/issues/16) | 🐕 펫 수아 슬라이스 (선아 미러) | AFK | #13, #14 | ✅ 완료 — 선아 미러 전수(레지스트리·`GAUGE_SUA`·버튼·티커·컬렉션 탭·`photo_sua_jirai`)·`ExpansionSlide` 펫 예고 소멸 |
...
- [~] **T32** 컬렉션북 탭 일반화 `(미호 탭=#5 ✅ · 펫 섹션=#6 ✅ · 메인/펫 그룹 토글=PR#17 ✅)` — 메인 4 + 펫 5 = 9탭. **#6에서 섹션 메타(`section`[main/pet/locked]) + `SECTION_GAP` 도입** → 캐릭터가 9명으로 늘며 탭 한 줄(327px)이 창(248px)을 넘쳐 가죽 프레임 침범 → **PR#17에서 메인/펫 그룹 토글로 분리**(탭 줄 좌측 `메인`/`펫` `GroupChip` → 한 그룹씩만 노출, `Characters.mains()/pets()` 파생으로 `TABS`/`SECTION_GAP` 하드코딩 폐기[단일 출처], 그룹별 마지막 본 캐릭터 복귀 `_last_main/_last_pet`, 평면 포커스 링에 `kind:"group"` — 3버튼만으로 전환 도달). 펫 5탭 끝 289<창 295 검증. 콘텐츠 270 통과(`_test_book_group_tabs` 신규). 남은 분: 잠긴 탭 **3-상태 수명**(미빌드 실루엣 → 빌드+무료 → 빌드+♡게이트) — `locked`/`ExpansionSlide` 분기는 PR#17에서 제거(클래스 파일 보존), **T39 ♡게이트로 부활**.
...
- [x] **T38** 멜 메인 본배선 `(멜=#14)` — **완료**: 옥자/미호/바나 미러(토대 재사용). 선행 배선(#15/#16 커밋 9c9edc8)으로 레지스트리·`GAUGE_MEL`·`events.mine` 참여·idle/체키 art 는 이미 있었고, 이번에 **본배선**: ① `buttons.json` `mel` emotion 전용(cheki=smile·drink=brew·touch=[shy,smile]·touch_cap=sad) + `characters.gd` `buttons:"okja"→"mel"`(라벨·순서는 okja.actions 공유 잠금, emotion만 전용) ② 멜 전용 대사 `ticker.json` `mel`+`mel_cutin` / `talk.json` `mel` / `gifts.json` `mel`(톤=단정·새초롬·동양적 강시, 모티프=부적·달·청운 에이드·강시 폴짝·반려견 선아/수아) ③ 시그니처 **청운 에이드** = drink→brew(`mel_brew` 청록 음료 잔, 별도 데이터 없이 brew 연출 공유) ④ 600 도달 멜 전용 반말 컷인(`mel_cutin.comfy`, 옥자·미호·바나 독립·말투 누설 0) ⑤ 컬렉션북 멜 탭 **메인 섹션** `locked:false`(바나 뒤, 잠긴 예고→실제 지뢰계 그리드) — 데모 전원 해제로 `ExpansionSlide` 미발생(코드 보존) ⑥ `docs/script-mel.md` 톤 가이드 신설. **검수**: 헤드리스 392통과(112+263+17, 멜 보이스 계약 14 + 탭 해제 단언 신규)·옥자·미호·바나·시온이·규종이·코코·선아·수아 회귀 0.
...
- [x] **T33+** 슬라이스 #1 미호 배선 `(라이브=#2 ✅ · 교감=#4 ✅ · 체키=#5 ✅ · 펫 규종이=#6 ✅)` → **바나(#10 ✅ T36) → 코코(#11 ✅ T37) → 멜(#14 ✅ T38, 펫 선아 #15 ✅ · 수아 #16 ✅)** — 각자 레지스트리 1항 + 에셋 + 대사 파일. **✅ 전 캐릭터 수직 슬라이스 완결**(메인 옥자·미호·바나·멜 + 펫 시온이·규종이·코코·선아·수아 / 추적 이슈 #7·#8·#12 전부 클로즈, 2026-06-10). 다음 = **T39 해금 경제(♡)**. **교감 파리티(#4) 완료**: `data/dialogue.gd` 캐릭터별 일반화(폴백 방식 — ticker 키 직접 / talk·gifts `_section` 폴백으로 옥자 평면 legacy 유지 → content_studio 무손상), 미호 전용 대사(`ticker.json` `miho`+`miho_cutin` / `talk.json` `miho` / `gifts.json` `miho`, 톤=포근·애교·영리한 여우 + 시그니처 '미호 스파클링'), 미호 600 도달 **전용 반말 컷인**(`stage_cutin` character 렌더 + `cutin(key)`), `cafe`(`_dialogue_key`)·`splash`(`_main_id` — 재방문 맞이도 active_main)·`characters`(미호 `dialogue:miho`) 배선. `is_casual` 은 stage 만 받아 character-agnostic(옥자/미호 말투 누설 없음 — 테스트 못박음). `docs/script-miho.md` 신설. **체키 파리티(#5) 완료**: 미호 탭 잠금 해제·마일스톤 전수(→ T34). **펫 규종이(#6) 완료**(→ T35, active_pet 정식 배선) — 미호 슬라이스(라이브·교감·체키·펫) 전부 완료.
- [ ] **T39(최후)** 해금 경제 — ♡ 화폐(데일리 교감 적립·HUD·세이브 필드) + 상점에서 확정 해금(펫 저렴). 잠긴 탭을 ♡게이트로 전환. **🟢 진행 가능**(전 캐릭터 추가 완료 — 2026-06-10). T32 잔여(잠긴 탭 3-상태 수명)와 결합. ※ 잠긴 탭/`ExpansionSlide` 분기는 PR#17(그룹 토글)에서 제거됐으니 `GroupChip`/그룹 구조 위에 ♡게이트 상태를 새로 얹는 형태로 재도입(클래스 파일은 보존됨).

**🎨 아트 트랙 (코드보다 항상 선행)**

- [~] 신규 메인 1명당: 표정 6장(idle/smile/shy/sad/brew/talk) + 초상 + 인트로(지뢰계) 체키 의상 1벌 → 이벤트 의상 점증 `(미호=#1)` — **미호·바나 ✅ 완료**(바나=뱀파이어 메이드 표정6·초상·지뢰계 의상 `bana_jirai`·체키 배경 `bg_cheki_bana_jirai`, 퍼플/네이비 램프), **멜 ✅ 아트 완료** `(멜=#13)`(강시 메이드=블루/틸 강시 관복·볼 빨간 점·관모·흑발 일자뱅, 표정 6종·초상·청운 에이드 brew·지뢰계 의상 `mel_jirai` + `bg_cheki_mel_jirai` 청록 야경 추가, 헤드리스 로드 검증. ⚠️ 검수 시 청록/틸 미드톤 1색 보강 후보. **메인 배선 #14 ✅ 완료**(→ T38) — 전용 대사·반말 컷인·컬렉션 탭 잠금 해제·청운 에이드 brew)
- [x] 신규 펫 1마리당: 반응 4컷(idle/간식/놀기/쓰담) + 초상 + 인트로 체키 베이크컷 `(규종이=#6)` — **규종이·코코·선아·수아 ✅ 완료**(코코=바나의 까맣고 마른 고양이; 선아=멜의 갈색 곱슬 푸들·`photo_suna_jirai`; 수아=멜의 베이지 장모 닥스훈트·반응 4컷 96×96·초상·지뢰계 베이크컷 `photo_sua_jirai`, 레퍼 `_src/sua_ref.png`). 펫 전원 정식 탭 합류 → `ExpansionSlide` 펫 예고 문구 소멸. 펫 아트 트랙 완결
- [ ] 시그니처 음료 연출 컷(블러디 미드나잇·청운 에이드·미호 스파클링) `(미호 스파클링=#1)` — 청운 에이드는 `mel_brew` 표정(청록 음료 잔) 겸용
