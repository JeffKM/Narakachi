...
# 크림 속지가 가죽 창으로 드러나는 영역(오버스캔 M=15 기준 측정값) — 카드는 이 안에 들어가야
# 가죽 테두리를 침범하지 않는다. x45~303(258) · y31~447.  (← 합성 측정, 2026-06-05)
const WINDOW := Rect2(45, 31, 258, 416)

# 그리드 — 창(258px) 중앙에 2열 120px 카드. margin 5 / gutter 8 (5+120+8+120+5=258).
const GRID_COLS := 2
const GRID_X := 50           # WINDOW.x(45) + margin 5
const GRID_Y := 84
...
func _build_book_frame() -> void:
  ...
  if tex:
    ...
    frame.position = Vector2(-FRAME_BLEED, -FRAME_BLEED)
...
func _build_butterflies() -> void:
  ...
  var spots := [
    {"pos": Vector2(282, 70), "hue": Palette.VIOLET},      # 우상단(탭 아래)
    {"pos": Vector2(56, 432), "hue": Palette.ACCENT_PINK}, # 좌하단(그리드 아래)
  ]
  for s in spots:
